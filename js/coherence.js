...
function controlesCoherence(pieces, ch) {
  ...
  pieces.forEach(p => {
    const d = p.dims || {};
    if (!d.l || !d.la) {
      alertes.push({ portee: 'logement', niveau:'attention', texte: p.nom + ' : dimensions non renseignées — peinture, sols et surfaces seront comptés à 0 € pour cette pièce.' });
      return; // sans cotes, les autres contrôles n'ont pas de sens
    }
    const surf = d.l * d.la;
    const smin = S.surfaceMin[p.id];
    if (smin && surf < smin) {
      alertes.push({ portee: 'logement', niveau:'attention', texte: p.nom + ' : ' + surf.toFixed(1) + ' m² — inhabituel pour ce type de pièce (généralement ≥ ' + smin + ' m²). Vérifiez longueur et largeur.' });
    }
    if (d.h && (d.h < S.hauteurMin || d.h > S.hauteurMax)) {
      alertes.push({ portee: 'logement', niveau:'info', texte: p.nom + ' : hauteur de ' + d.h + ' m — atypique (habituellement entre ' + S.hauteurMin + ' et ' + S.hauteurMax + ' m).' });
    }
    if ((d.portes || 0) > S.portesMax) {
      alertes.push({ portee: 'logement', niveau:'info', texte: p.nom + ' : ' + d.portes + ' portes — nombre élevé, il conditionne le type de commande d\'éclairage (va-et-vient, télérupteur).' });
    }
    if ((d.fenetres || 0) > S.fenetresMax) {
      alertes.push({ portee: 'logement', niveau:'info', texte: p.nom + ' : ' + d.fenetres + ' fenêtres — vérifiez la saisie.' });
    }
    const ratio = Math.max(d.l, d.la) / Math.max(0.1, Math.min(d.l, d.la));
    if (ratio > S.ratioAllonge && p.id !== 'couloir') {
      alertes.push({ portee: 'logement', niveau:'info', texte: p.nom + ' : pièce très allongée (' + d.l + ' × ' + d.la + ' m) — longueur et largeur ne sont-elles pas inversées avec autre chose ?' });
    }
  });
  ...
  pieces.forEach(p => {
    ...
    if ((d.portes || 0) === 0 && !EXT.includes(p.id)
      ...
      alertes.push({ portee: 'logement', niveau:'info', texte: p.nom + ' : aucune porte indiquée — vérifiez la saisie (le nombre d\'accès détermine simple allumage / va-et-vient / télérupteur).' });
    }
    ...
    if (configuree && typeof metiersActifs !== 'undefined' && metiersActifs.includes('electricite')) {
      ...
      if (nbEcl === 0 && !EXT.includes(p.id)) {
        alertes.push({ portee: 'piece', niveau:'attention', texte: p.nom + ' : aucun point d\'éclairage prévu — inhabituel pour cette pièce.' });
      }
      ...
      if (nbPrises === 0 && ['salon','salle_manger','chambre','cuisine','bureau','sdb','sde','entree'].includes(p.id)) {
        alertes.push({ portee: 'piece', niveau:'attention', texte: p.nom + ' : aucune prise prévue — est-ce volontaire ?' });
      }
    }
    ...
    if (configuree && typeof metiersActifs !== 'undefined' && metiersActifs.includes('plomberie')) {
      ...
      if (p.id === 'cuisine' && points === 0) {
        alertes.push({ portee: 'piece', niveau:'info', texte: 'Cuisine : aucun point d\'eau (évier) prévu — est-ce volontaire ?' });
      }
      if ((p.id === 'sdb' || p.id === 'sde') && points === 0) {
        alertes.push({ portee: 'piece', niveau:'info', texte: p.nom + ' : aucun point d\'eau prévu (douche, baignoire, vasque) — est-ce volontaire ?' });
      }
    }
    ...
    if (typeof metiersActifs !== 'undefined' && metiersActifs.includes('peinture') && aPeinture && !(d.l && d.la)) {
      alertes.push({ portee: 'piece', niveau:'attention', texte: p.nom + ' : peinture prévue mais dimensions manquantes — les surfaces seront comptées à 0 €.' });
    }
  });
  ...
  if (surfDeclaree > 0 && surfSaisie > 0) {
    ...
    if (pct >= S.ecartSurfaceJaune) {
      ...
      alertes.push({ portee: 'logement', niveau: niveau, texte: chiffres + phrase });
    }
  }
  ...
  if (surfDeclaree >= S.grandeSurface && nbChambres > 0 && nbChambres < S.chambresMinGrandeSurface) {
    alertes.push({ portee: 'logement', niveau:'info', texte: 'Votre configuration comporte ' + nbChambres + ' chambre(s) pour ' + surfDeclaree + ' m². Vérifiez que toutes les pièces ont bien été renseignées.' });
  }

  // Contrôle final du moteur plomberie (doublons, incohérences, orphelins) — MISSION 030
  if (typeof verifierPlomberie === 'function') {
    verifierPlomberie(pieces).forEach(a => { a.portee = a.portee || 'piece'; alertes.push(a); });
  }
  // Contrôle final du moteur sols (surface, référence, dépose, sous-couche, doublons) — MISSION 033
  if (typeof verifierSols === 'function') {
    verifierSols(pieces).forEach(a => { a.portee = a.portee || 'piece'; alertes.push(a); });
  }
  // Contrôle final du moteur VMC (bouches, entrées d'air, cohérence caisson) — MISSION 040
  if (typeof verifierVMC === 'function') {
    verifierVMC(pieces).forEach(a => { a.portee = a.portee || 'piece'; alertes.push(a); });
  }
  ...
  if (typeof verifierMenuiserie === 'function') {
    verifierMenuiserie(pieces).forEach(a => { a.portee = a.portee || 'piece'; alertes.push(a); });
  }
  ...
  if (typeof verifierChauffage === 'function') {
    verifierChauffage(pieces, ch).forEach(a => { a.portee = a.portee || 'piece'; alertes.push(a); });
  }
  ...
  if (ch && ch.intentionVentilation === 'remplacer') {
    if (ch.typeVentilationExistante === 'aucune') {
      alertes.push({ portee: 'logement', niveau:'info', texte: 'Votre réponse indique qu\'il n\'y a pas actuellement de système de ventilation, mais vous avez indiqué vouloir le remplacer. Ce point devra être confirmé.' });
    } else if (ch.typeVentilationExistante === 'inconnu') {
      alertes.push({ portee: 'logement', niveau:'info', texte: 'Vous souhaitez remplacer votre ventilation, mais le système existant n\'est pas encore établi. Ce point devra être confirmé lors de la visite.' });
    }
  }
  ...
  if (ch && (ch.solutionVentilation === 'double_flux' || ch.solutionVentilation === 'inconnue')) {
    ...
    if (ch.solutionVentilation === 'double_flux') {
      alertes.push({ portee: 'logement', niveau:'attention', texte: 'Vous avez indiqué souhaiter une VMC double flux : cette solution n\'est pas encore entièrement chiffrée par DS.BAT et devra être étudiée puis confirmée lors de la visite. Le montant affiché ne comprend pas l\'installation double flux.' });
    } else {
      alertes.push({ portee: 'logement', niveau:'info', texte: 'Votre solution de ventilation n\'est pas encore définie : elle sera précisée et confirmée lors de la visite (aucune configuration VMC n\'est chiffrée à ce stade).' });
    }
  }
